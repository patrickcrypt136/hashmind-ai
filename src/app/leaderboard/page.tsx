  ...
  return (
    <div className="flex min-h-screen bg-[#0A0A0A]">
      ...
      <main className="flex-1 md:ml-60 p-4 md:p-8 pt-16 md:pt-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-xl md:text-2xl font-bold text-white">Leaderboard</h1>
        ...
        {loading ? (
          ...
          <>
                  ...
                  >
                    <div className="flex justify-center mb-3">
                      {i === 0 ? (
                        ...
                        <Medal
                          className={`w-8 h-8 ${rankColors[i] || "text-gray-400"}`}
                        />
                      )}
                    </div>
                    <div
                      className={`text-2xl font-bold mb-1 ${
                        rankColors[i] || "text-white"
                      }`}
                    >
                      #{i + 1}
                    </div>
            ...
            >
              {/* Desktop header */}
              <div className="hidden sm:grid grid-cols-5 px-6 py-3 border-b border-[#1F1F1F] bg-[#0A0A0A]">
                ...
                <>
                  ...
                  {players.map((player, i) => (
                    <motion.div
                      ...
                      className={`hidden sm:grid grid-cols-5 px-6 py-4 border-b border-[#1F1F1F] last:border-0 hover:bg-[#1F1F1F]/30 transition-colors ${
                        player.address === address
                  ...
                  {players.map((player, i) => (
                    <div
                      key={`mobile-${player.address}`}
                      className={`sm:hidden flex items-center justify-between px-4 py-3 border-b border-[#1F1F1F] last:border-0 ${
                        player.address === address
                          ? "bg-[#FF0033]/5 border-l-2 border-l-[#FF0033]"
                          : ""
                      }`}
                    >
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-gray-500">
                          {player.daily_reward.toFixed(2)}/day
                        </div>
                      </div>
                    </div>
                  ))}
